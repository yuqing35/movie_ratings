...
* [IMDb Datasets](https://www.imdb.com/interfaces/)         
  ...
  + Each dataset is in a gzipped, tsv formatted file in the UTF-8 character set. We are going to import using [ggplot2movies](https://cran.r-project.org/web/packages/ggplot2movies/index.html) package in R. 
  + The data is refreshed daily. 
  +We consider using this source because it contains up-to-date movie ratings collected directly from the users. In addition to movie ratings, it also provides information such as the title, region, language, release year, genres associated with the title, number of votes, etc. Then we’ll try to visualize the relationship between each factor and ratings. For example, we will compare movies with different genres (e.g., whether documentary movies would generally have higher ratings). 



* [MovieLens 25M Dataset](https://grouplens.org/datasets/movielens/)
  + These datasets were collected from MovieLens by GroupLens. GroupLens is a research lab in the Department of Computer Science and Engineering at the University of Minnesota. 
  + There are 6 different datasets, which contain over 25 million titles of movies and their genres, the ratings of the movies, and so on. 
  + Because the variety and number of movies in this dataset are large, the results we got from this dataset would be more accurate and more convincing. And we consider analyzing the genres 
  + The format of this dataset is ‘csv’ file, which is easy to read by using ‘readr’ package in RStudio. 
